...
- `porntrex.com`
- `www.porntrex.com`
- CDN: `cdntrex.com`, `statics.cdntrex.com` (thumbs / media redirects)
...
Seed from nav: Home, Latest, Top Rated, Most Viewed, plus popular category slugs (4K, Amateur, Teen, Milf, Anal, Lesbian, Blowjob, Hardcore).

### Registration checklist for PornTrex

Besides creating `backend/app/scrapers/porntrex/`, update all of these:
...
- `backend/app/main.py`
  ...
  - `_scrape_dispatch`
  - `_list_dispatch`
  - `/api/v1/categories` source mapping (`source=porntrex`)
- `backend/app/services/video_streaming.py`
  - scraper selection branch
  - supported-host help text
  - stream quality map host checks for `porntrex.com` and `cdntrex.com`
- `backend/app/api/endpoints/explore.py`
  - add `ExploreSourceResponse` entry (`sourceId="porntrex"`)
