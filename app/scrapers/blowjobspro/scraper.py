...
async def fetch_page(url: str, referer: str = "https://blowjobs.pro/") -> str:
    headers = {
...
def _clean_title(title: str | None) -> Optional[str]:
    if not title:
        return None
    t = str(title).strip()
    for suffix in (" | Blowjobs.pro", " - Blowjobs.pro"):
...
def _normalize_video_href(href: str) -> Optional[str]:
    ...
    if href.startswith("//"):
        ...
        href = f"https://blowjobs.pro{href}"
    if not href.startswith("http"):
        return None

    parsed = urlparse(href)
    if "blowjobs.pro" not in parsed.netloc.lower():
        return None
    ...
    return urlunparse(("https", "blowjobs.pro", parsed.path.rstrip("/") + "/", "", "", ""))
...
def parse_video_page(html: str, url: str) -> dict[str, Any]:
    ...
    # Usually appears as "... 17:48 304 704 1 month ago ..."
...
def _build_list_page_url(base_url: str, page: int) -> str:
    ...
    netloc = p.netloc or "blowjobs.pro"
    path = p.path or "/"
...
async def list_videos(base_url: str, page: int = 1, limit: int = 100) -> list[dict[str, Any]]:
    ...
    try:
        html = await fetch_page(page_url, referer=base_url or "https://blowjobs.pro/")
    except Exception:
        return []
