...
async def fetch_page(url: str) -> str:
    headers = {
        ...
        "Referer": "https://indianporn365.xyz/",
    }
...
def _clean_title(title: str | None) -> Optional[str]:
    if not title:
        return None
    t = title.strip()
    for suffix in (" - Indian Porn 365", " | Indian Porn 365", " - IndianPorn365", " | IndianPorn365"):
...
def _normalize_video_href(href: str) -> Optional[str]:
    ...
    if href.startswith("//"):
        ...
        href = f"https://indianporn365.xyz{href}"
    if not href.startswith("http"):
        return None

    parsed = urlparse(href)
    if "indianporn365.xyz" not in parsed.netloc.lower():
        return None
    if any(
    ...
    return urlunparse(("https", "indianporn365.xyz", f"/{slug}/", "", "", ""))
...
def _build_list_page_url(base_url: str, page: int) -> str:
    ...
    netloc = parsed.netloc or "indianporn365.xyz"
    path = parsed.path or "/"
