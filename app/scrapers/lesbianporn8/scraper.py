...
def can_handle(host: str) -> bool:
    h = (host or "").lower()
    return h == "lesbianporn8.net" or h.endswith(".lesbianporn8.net")
...
async def fetch_page(url: str, referer: str = "https://lesbianporn8.net/") -> str:
    headers = {
...
def _clean_title(title: str | None) -> Optional[str]:
    if not title:
        return None
    t = str(title).strip()
    for suffix in (" | LesbianPorn8.com", " - LesbianPorn8.com", " | Lesbian Porn 8", " - Lesbian Porn 8"):
...
def _normalize_video_href(href: str) -> Optional[str]:
    ...
    if href.startswith("//"):
        ...
        href = f"https://lesbianporn8.net{href}"
    if not href.startswith("http"):
        return None

    parsed = urlparse(href)
    if "lesbianporn8.net" not in parsed.netloc.lower():
        return None
    if not re.match(r"^/videos/\d+/[^/]+/?$", parsed.path or "", flags=re.IGNORECASE):
        return None
    if parsed.query:
        return None
    return urlunparse(("https", "lesbianporn8.net", parsed.path.rstrip("/") + "/", "", "", ""))
...
def _extract_native_embed_url(html: str, video_url: str) -> Optional[str]:
    m = re.search(r"https?://(?:www\.)?lesbianporn8\.net/embed/\d+\b", html, flags=re.IGNORECASE)
    if m:
        return m.group(0).strip()
    vm = re.search(r"/videos/(\d+)/", video_url)
    if vm:
        return f"https://lesbianporn8.net/embed/{vm.group(1)}"
    return None
...
def _extract_streams(soup: BeautifulSoup, html: str, video_url: str) -> dict[str, Any]:
    ...
    if native_embed and native_embed not in seen:
        ...
        streams.append({"url": native_embed, "quality": "lesbianporn8", "format": "embed"})

    uniq = list(dict.fromkeys((json.dumps(s, sort_keys=True) for s in streams)))
    materialized = [json.loads(s) for s in uniq]
    materialized.sort(key=lambda s: ("lesbianporn8.net/embed/" in (s.get("url") or "").lower()), reverse=True)
...
def _build_list_page_url(base_url: str, page: int) -> str:
    ...
    netloc = p.netloc or "lesbianporn8.net"
    path = p.path or "/"
...
async def list_videos(base_url: str, page: int = 1, limit: int = 100) -> list[dict[str, Any]]:
    ...
    try:
        html = await fetch_page(page_url, referer=base_url or "https://lesbianporn8.net/")
    except Exception:
        return []
    soup = BeautifulSoup(html, "lxml")
