...
def _candidate_url_score(stream_url: str, slug_tokens: set[str]) -> tuple[int, int]:
    ...
    cdn_score = 2 if "cdn.thekamababa.com" in low or "cdn.kamababa" in low else 0
    token_hits = sum(1 for t in slug_tokens if t in low)
    return (cdn_score + min(token_hits, 3), token_hits)


def _normalize_media_url(src: str, base: str = "https://www.thekamababa.com/") -> Optional[str]:
    u = (src or "").strip()
    if not u:
        return None
    if u.startswith("//"):
...
def _build_list_page_url(base_url: str, page: int) -> str:
    ...
    netloc = parsed.netloc or "www.thekamababa.com"
    path = parsed.path or "/"
