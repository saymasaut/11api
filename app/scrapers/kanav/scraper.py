...
BASE_SITE = "https://kanav.ad/"
SITE_HOST = "kanav.ad"
SITE_ALIASES = frozenset({"kanav.ad", "www.kanav.ad"})

_PLAY_PAGE_RE = re.compile(
    r"^https?://(?:www\.)?kanav\.ad/index\.php/vod/play/id/(?P<vid>\d+)/",
    re.IGNORECASE,
)
_DETAIL_PAGE_RE = re.compile(
    r"^https?://(?:www\.)?kanav\.ad/index\.php/vod/detail/id/(?P<vid>\d+)\.html",
    re.IGNORECASE,
)
...
def _clean_title(title: str | None) -> Optional[str]:
    if not title:
        return None
    t = str(title).strip()
    for suffix in (" - KanAV", " | KanAV", " - KanAV-免费高清中文AV在线看"):
        if suffix in t:
...
def _canonical_play_url(video_id: str) -> str:
    return f"https://{SITE_HOST}/index.php/vod/play/id/{video_id}/sid/1/nid/1.html"
...
def _streams_from_player(html: str) -> dict[str, Any]:
    ...
    if url_field and encrypt_val == "2":
        decoded = _decode_encrypt2_url(str(url_field))
...
def _parse_list_items(soup: BeautifulSoup, *, limit: int) -> list[dict[str, Any]]:
    ...
    for a in soup.select('a[href*="/index.php/vod/play/id/"]'):
        ...
        title = a.get_text(strip=True) or str(a.get("title") or "").strip()
        img = a.find("img")
        if not title and img:
            title = str(img.get("alt") or img.get("title") or "").strip()
        thumb = _best_image_url(img)

        prev = by_id.get(vid)
        if prev:
            if len(title) > len(prev.get("title") or ""):
                prev["title"] = title
...
def _build_list_page_url(base_url: str, page: int) -> str:
    ...
    if m:
        ...
        # Home and other non-type listings: MacCMS often uses /index.php/vod/show/id/0/page/N.html
        # KanAV home embeds sections; prefer type URLs for pagination. Fallback: append page segment.
        if path in ("/", ""):
...
async def scrape(url: str) -> dict[str, Any]:
    ...
    if not video_id:
        raise ValueError(f"Unsupported KanAV URL: {url}")
