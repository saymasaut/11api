...
async def fetch_page(url: str, referer: str = "https://blackporn24.com/") -> str:
    headers = {
...
def _clean_title(title: str | None) -> Optional[str]:
    if not title:
        return None
    t = str(title).strip()
    for suffix in (" | BlackPorn24.com", " - BlackPorn24.com", " | Black Porn 24", " - Black Porn 24"):
...
def _normalize_video_href(href: str) -> Optional[str]:
    ...
    if href.startswith("//"):
        ...
        href = f"https://blackporn24.com{href}"
    if not href.startswith("http"):
        return None

    parsed = urlparse(href)
    if "blackporn24.com" not in parsed.netloc.lower():
        return None
...
def _extract_native_embed_url(html: str, video_url: str) -> Optional[str]:
    m = re.search(r"https?://(?:www\.)?blackporn24\.com/embed/\d+\b", html, flags=re.IGNORECASE)
    if m:
        return m.group(0).strip()
    vm = re.search(r"/videos/(\d+)/", video_url)
    if vm:
        return f"https://blackporn24.com/embed/{vm.group(1)}"
    return None
...
def _build_list_page_url(base_url: str, page: int) -> str:
    ...
    netloc = p.netloc or "blackporn24.com"
    path = p.path or "/"
...
async def list_videos(base_url: str, page: int = 1, limit: int = 100) -> list[dict[str, Any]]:
    ...
    try:
        html = await fetch_page(page_url, referer=base_url or "https://blackporn24.com/")
    except Exception:
        return []
