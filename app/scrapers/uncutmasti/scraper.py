...
BASE_SITE = "https://uncutmasti.com/"
SITE_HOST = "uncutmasti.com"
SITE_ALIASES = frozenset({"uncutmasti.com", "www.uncutmasti.com"})
...
_POST_PAGE_RE = re.compile(
    r"^https?://(?:www\.)?uncutmasti\.com/(?P<slug>[a-z0-9][a-z0-9-]*)/?$",
    re.IGNORECASE,
)
_MP4_RE = re.compile(
    r"https?://[^\s\"'<>]+\.mp4(?:\?[^\s\"'<>]*)?",
...
def can_handle(host: str) -> bool:
    ...
    if h.startswith("www."):
        h = h[4:]
    return h in SITE_ALIASES or h.endswith(".uncutmasti.com")
...
def _normalize_views(text: str | None) -> Optional[str]:
    ...
    if raw.upper().endswith("K"):
        ...
        if digits:
            return str(int(float(digits) * 1000))
...
def _streams_from_html(html: str) -> dict[str, Any]:
    ...
    for source in soup.select("video source[src], video[src]"):
        ...
        if not raw:
            continue
        url = html_lib.unescape(str(raw).strip())
    ...
    for pat, fmt in ((_M3U8_RE, "hls"), (_MP4_RE, "mp4")):
        for raw in pat.findall(html_norm):
            url = html_lib.unescape(raw.strip().rstrip("/"))
            if not url.startswith("http") or url in seen or _is_preview_url(url):
                continue
            seen.add(url)
            streams.append({"url": url, "quality": "adaptive" if fmt == "hls" else "source", "format": fmt})
...
def _parse_list_items(soup: BeautifulSoup, *, limit: int) -> list[dict[str, Any]]:
    ...
    for block in soup.select("article.thumb-block, .thumb-block.video-preview-item"):
        ...
        if not title:
            for sel in (".entry-header a", "h2 a", "h3 a", "a[title]"):
...
def parse_video_page(html: str, url: str, *, video: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    duration = None
    dur_el = soup.select_one("span.duration, .duration")
    if dur_el:
...
async def scrape(url: str) -> dict[str, Any]:
    ...
    if not canon:
        raise ValueError(f"Unsupported UncutMasti URL: {url}")
