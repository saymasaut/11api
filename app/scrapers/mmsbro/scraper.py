...
def _clean_title(title: str | None) -> Optional[str]:
    if not title:
        return None
    t = title.strip()
    for suffix in (" - MmsBro", " | MmsBro", " - MMSBro", " | MMSBro"):
...
def _normalize_video_href(href: str) -> Optional[str]:
    ...
    if href.startswith("//"):
        ...
        href = f"https://mmsbro.com{href}"
    if not href.startswith("http"):
        return None

    parsed = urlparse(href)
    if "mmsbro.com" not in parsed.netloc.lower():
        return None
    if any(
        x in parsed.path.lower()
        for x in ("/wp-content/", "/wp-json/", "/tag/", "/category/", "/page/", "/author/", "/feed/")
    ):
        return None
    if parsed.query:
        return None
    ...
    return urlunparse(("https", "mmsbro.com", f"/{slug}/", "", "", ""))
...
def _normalize_media_url(src: str, base: str = "https://mmsbro.com/") -> Optional[str]:
    u = (src or "").strip()
    if not u:
        return None
    if u.startswith("//"):
...
def _build_list_page_url(base_url: str, page: int) -> str:
    ...
    netloc = parsed.netloc or "mmsbro.com"
    path = parsed.path or "/"
