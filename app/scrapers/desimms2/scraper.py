...
async def fetch_page(url: str) -> str:
    headers = {
        ...
        "Referer": "https://www.desimms2.site/",
    }
...
def _clean_title(title: str | None) -> Optional[str]:
    if not title:
        return None
    t = title.strip()
    for suffix in (" - Desi MMS Site", " | Desi MMS Site", " - DesiMMS2", " | DesiMMS2"):
...
def _normalize_video_href(href: str) -> Optional[str]:
    ...
    if href.startswith("//"):
        ...
        href = f"https://www.desimms2.site{href}"
    if not href.startswith("http"):
        return None

    parsed = urlparse(href)
    if "desimms2.site" not in parsed.netloc.lower():
        return None
    if any(
    ...
    return urlunparse(("https", "www.desimms2.site", f"/{slug}/", "", "", ""))
...
def _build_list_page_url(base_url: str, page: int) -> str:
    ...
    netloc = parsed.netloc or "www.desimms2.site"
    path = parsed.path or "/"
