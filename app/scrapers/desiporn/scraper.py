...
async def fetch_page(url: str, referer: str = "https://desiporn.one/") -> str:
    headers = {
...
def _clean_title(title: str | None) -> Optional[str]:
    if not title:
        return None
    t = str(title).strip()
    for suffix in (" | DesiPorn", " - DesiPorn", " | Desi porn", " - Desi porn"):
...
def _normalize_video_href(href: str) -> Optional[str]:
    ...
    if href.startswith("//"):
        ...
        href = f"https://desiporn.one{href}"
    if not href.startswith("http"):
        return None

    parsed = urlparse(href)
    if "desiporn.one" not in parsed.netloc.lower():
        return None
    ...
    return urlunparse(("https", "desiporn.one", parsed.path.rstrip("/") + "/", "", "", ""))
...
def _build_list_page_url(base_url: str, page: int) -> str:
    ...
    netloc = p.netloc or "desiporn.one"
    path = p.path or "/"
...
async def list_videos(base_url: str, page: int = 1, limit: int = 100) -> list[dict[str, Any]]:
    ...
    try:
        html = await fetch_page(page_url, referer=base_url or "https://desiporn.one/")
    except Exception:
        return []
