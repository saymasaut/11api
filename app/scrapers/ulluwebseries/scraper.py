...
def _clean_title(title: str | None) -> Optional[str]:
    ...
    # collapse duplicated trailing site name from <title>
...
def _parse_list_items(soup: BeautifulSoup, *, limit: int) -> list[dict[str, Any]]:
    ...
    for block in soup.select("div.pt-cv-content-item"):
        if len(items) >= limit:
            break
        thumb_link = block.select_one("a.pt-cv-href-thumbnail[href], .pt-cv-thumb-wrapper a[href]")
        title_link = block.select_one("h4.pt-cv-title a[href], .pt-cv-title a[href]")
        ...
        img = block.select_one("img.pt-cv-thumbnail, img")
        title = _first_non_empty(
            title_link.get_text(" ", strip=True) if title_link else None,
            title_link.get("title") if title_link else None,
            thumb_link.get("title") if thumb_link else None,
    ...
    if len(items) < limit:
        for a in soup.select('a[href*="/hot-series/"]'):
            if len(items) >= limit:
                break
...
def parse_video_page(html: str, url: str, *, video: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    thumbnail = _first_non_empty(
        ...
        _best_image_url(soup.select_one("video, img.pt-cv-thumbnail, img.wp-post-image, img")),
    )
    ...
    for a in soup.select(
        'a[rel="tag"], a[href*="/series_category/"], a[href*="/category/"], a[href*="/tag/"]'
    ):
...
async def scrape(url: str) -> dict[str, Any]:
    ...
    if not canon:
        raise ValueError(f"Unsupported Ullu Web Series URL: {url}")
